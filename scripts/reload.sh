...
APP_NAME="Crucible DEV"
BUNDLE_ID="com.cmuxterm.app.debug"
BASE_APP_NAME="Crucible DEV"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
DERIVED_SET=0
...
reload_cleanup_tag_state_with_lock() {
  ...
  perl -MFcntl=:DEFAULT -MFcntl=:flock -MSocket -MIO::Select -MFile::Basename=dirname -MFile::Temp=tempfile -MErrno=EEXIST,ECONNREFUSED,ENOENT,EAGAIN,EWOULDBLOCK,EINPROGRESS,EALREADY,EISCONN -e '
    ...
    my $cli_suffix = "/Crucible DEV ${slug}.app/Contents/Resources/bin/cmux";
...
cleanup_stale_cli_pointer_target() {
  ...
  [[ "$app_name" == "Crucible DEV "* ]] || return 0
  local slug="${app_name#Crucible DEV }"
...
bundle_socket_path() {
  ...
  local app_name="\${bundle_path##*/}"
  app_name="\${app_name%.app}"
  if [[ "\$app_name" == "Crucible DEV "* ]]; then
    local tag="\${app_name#Crucible DEV }"
...
if [[ -n "\$SOCKET_ARG" ]]; then
  ...
  if [[ "\$SOCKET_NAME" == cmux-debug-*.sock ]]; then
    ...
    if [[ "\$TAG" =~ ^[A-Za-z0-9_-]+$ ]]; then
      TAG_CLI="\$HOME/Library/Developer/Xcode/DerivedData/cmux-\$TAG/Build/Products/Debug/Crucible DEV \$TAG.app/Contents/Resources/bin/cmux"
...
print_tag_cleanup_reminder() {
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"Crucible DEV ${tag}.app/Contents/MacOS/Crucible DEV\""
  ...
  echo "  pkill -f \"Crucible DEV ${current_slug}.app/Contents/MacOS/Crucible DEV\""
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="Crucible DEV ${TAG_SLUG}"
  fi
