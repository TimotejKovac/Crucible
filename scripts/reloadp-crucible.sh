...
pkill -x Crucible || true
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/Crucible.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "Crucible.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/Crucible"
ATTEMPT=0
MAX_ATTEMPTS=20
