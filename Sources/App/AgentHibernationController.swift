...
struct AgentHibernationRecord {
    ...
    // Upstream tip does not compile: a `let` with a default is excluded from the
    // memberwise init, but +Records.swift passes `processLiveness:`. `var` keeps
    // the default while accepting the argument.
