...
class TabManager: ObservableObject {
    ...
    /// Crucible fork: auto-incrementing workspace number for default naming.
    private var nextWorkspaceNumber: Int = 1

    /// Crucible fork: set after creating a workspace that should immediately show the rename dialog.
    ...
    func addWorkspaceIfActive(
        ...
        return withExtendedLifetime((capturedTabs, sourceWorkspace)) {
            ...
            case .terminal:
                // Crucible fork: number terminal workspaces independently of tab count.
                let workspaceNumber = nextWorkspaceNumber
                nextWorkspaceNumber += 1
                defaultTitle = "Workspace #\(workspaceNumber)"
            case .browser:
            ...
            // Crucible fork: auto-assign a color in a fixed rotation, skipping colors already in use.
            do {
                let colorRotation = [
                    "#C0392B", // Red
                    "#7D6608", // Amber (yellow)
            ...
            // Crucible fork: prompt rename for new terminal workspaces except the very first one.
            if initialSurface == .terminal, snapshot.tabs.count > 0 {
