...
struct ContentView: View {
    ...
    // Crucible fork: right-hand browser shortcut panel.
    ...
    private var sidebarView: some View {
        ...
        // Crucible fork: 1px separator on the sidebar's trailing edge.
    ...
    // MARK: - Crucible fork: right-hand browser shortcut panel
    ...
    private func contentAndSidebarLayout(appearance: WindowAppearanceSnapshot) -> AnyView {
        ...
        return AnyView(
            // Crucible fork: the browser shortcut bar rides the trailing edge of
            // every layout mode, shifting the content left by its width. The
            // resizer overlays stay on `layout` so their geometry matches the
            // content area they position against.
            HStack(spacing: 0) {
                layout
                    .overlay(alignment: .leading) {
                        if sidebarState.isVisible {
            ...
            .onChange(of: tabManager.pendingRenameWorkspaceId) { _, newValue in
                presentPendingWorkspaceRenamePrompt(newValue)
            }
        )
    }

    // Crucible fork: rename dialog shown right after creating a workspace
    // (armed by TabManager.pendingRenameWorkspaceId).
    private func presentPendingWorkspaceRenamePrompt(_ workspaceId: UUID?) {
...
// Crucible fork: model + row button for the right-hand browser shortcut panel.
